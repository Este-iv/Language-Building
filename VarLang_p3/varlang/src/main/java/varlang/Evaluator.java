...
//			values.add((Value)exp.accept(this, env));
		...
		for (int i = 0; i < names.size(); i++)
			new_env = new ExtendEnv(new_env, names.get(i), (Value) value_exps.get(i).accept(this,new_env));
	...
	public Value visit(DefineDecl d, Env env) {
		String identifier = d.getIdent();
		Exp exp = d.getExp();
		Value value = (Value) exp.accept(this, env);
		initEnv = new ExtendEnv(initEnv, identifier, value);
